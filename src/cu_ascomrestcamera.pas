...
begin
 try
  ...
  if V.Get('connected').AsBool then begin
    ...
    try
      FcanStopExposure:=V.Get('canstopexposure').AsBool
    except
...
procedure T_ascomrestcamera.StopExposure;
begin
  try
...
function T_ascomrestcamera.GetVideoRecordRunning: boolean;
begin
 result:=false;
 // todo
end;
...
function T_ascomrestcamera.GetVideoPreviewLimit:integer;
begin
 result:=0;
 // todo
end;

procedure T_ascomrestcamera.SetVideoPreviewLimit(value:integer);
begin
 // todo
end;
...
function  T_ascomrestcamera.GetAperture:double;
begin
 // ASCOM Aperture in mount driver
result:=-1;
end;

function  T_ascomrestcamera.GetFocaleLength:double;
begin
