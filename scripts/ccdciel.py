...
class CCDcielException(Exception):
    def __init__(self, message: str):
        self.message = message
        super().__init__(f'CCDciel error: {self.message}')
...
# CCDciel method that raise an exception in case of error
def ccdcielex(method, params='', host=None, port=None):

    # Call the default method
    result = ccdciel(method, params, host, port)

    # Check Json-Rpc error
    if ('error' in result):
      if isinstance(result['error'], dict):
         raise JsonRpcException(result['error']['code'], result['error']['message'])
